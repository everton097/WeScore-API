...
exports.disenrollJogador = async (req, res) => {
	try {
		...
		res
			.status(200)
			.json({ message: "Time desassociado ao jogador com sucesso." });
	} catch (error) {
		console.error("Erro ao desassociar time ao jogador:", error);
		res.status(500).json({ error: "Erro interno do servidor." });
	}
};
exports.enrollJogador = async (req, res) => {
	try {
		const { idJogador, idTime } = req.body;
		...
		// Associar o time ao jogador (presumindo que você tem uma associação entre Jogador e Time)
		jogador.idTime = idTime;
		await jogador.save();

		res.status(200).json({ message: "Time associado ao jogador com sucesso." });
	} catch (error) {
		console.error("Erro ao associar time ao jogador:", error);
		res.status(500).json({ error: "Erro interno do servidor." });
	}
};
exports.getJogadorNoLink = async (req, res) => {
	try {
		const jogador = await Jogador.findAll({
			where: { idTime: null },
		});
